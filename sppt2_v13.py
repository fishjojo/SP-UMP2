...
class SPPT2:
    ...
    def get_matrix_element_v2(self, mo_coeff1, mo_coeff2, mo_occ1, mo_occ2, verbose=False):
        """
        Computes the matrix elements 
            1. <Psi_{UHF}|H R|Psi_{UHF}>
            2. <Psi_{UHF}|H R|Psi_{UHF}^{ab}_{ij}>
    ...
    def get_matrix_element_v3(self, dfgmp2, mo_coeff1, mo_coeff2, mo_occ, verbose=False):
        """
        Computes the matrix element <Psi_{UHF}|H R|Psi_{UHF}>.
        ...
        """
        if not isinstance(self.mo_eri, numpy.ndarray):
            self.get_mo_eri_with_df(dfgmp2, mo_coeff1)
        ...
        #
        # sum{ h_ik rho_ki + 1/2 <ij||kl> rho_ki rho_lj}
        part1 = numpy.einsum('ikjl, ki->jl', self.mo_eri_df, trans_rdm1)
        part2 = numpy.einsum('iljk, ki->jl', self.mo_eri_df, trans_rdm1)
        term = numpy.einsum('ik, ki', mo_hcore, trans_rdm1) + 0.5 * (
               numpy.einsum('jl, lj', part1, trans_rdm1) - numpy.einsum('jl, lj', part2, trans_rdm1))
    ...
    def get_S_ijpr(self, co_ovlp, co_ovlp_oo_diag, inds):
        """
        Computes the (i, j, p, r) element of the S_ijpr tensor used in the evaluation of 
        <Psi_{UHF}|H R|Psi_{UHF}^{1}>.
    ...
    def get_S_ipr(self, co_ovlp, co_ovlp_oo_diag, inds):
        """
        Computes the (i, p, r) element of the S_ijpr tensor used in the evaluation of 
        <Psi_{UHF}|H R|Psi_{UHF}^{1}>.
    ...
    def energy_01_v2(self, ump2, mo_coeff2):
        ...
        - No CO transformation for vv block.
        - Transform t_ijab with V, V^\dagger.
        - Compute mo_eri once with df, use `get_co_from_mo_eri_v2`.
        """
        nocc = self.nelec
        nvir = self.nmo - nocc
        ...
        co_coeff1 = self.get_co_coeff(mo_coeff1, u)
        co_t2 = self.get_co_t2(mo_t2, v_oo)
        co_eri = self.get_co_eri_from_mo_eri_with_df(ump2, mo_coeff1, u_oo)
    ...
    def energy_v2(self, s, m, k, ump2, proj='part', N_alpha=None, N_beta=None, N_gamma=None, 
        ...
        if proj == 'part':
            for b in range(N_beta):
                rot_mo_coeff = rot_mo_coeffs[b]
                coeff = coeffs[b]
                _energy_00 = self.get_matrix_element_v3(ump2, mo_coeff, rot_mo_coeff, mo_occ, verbose=verbose)
                _norm_00 = self.det_ovlp(mo_coeff, rot_mo_coeff, mo_occ, mo_occ)
                energy_00 += coeff * _energy_00
                norm_00 += coeff * _norm_00
    ...
    def energy_v4(self, s, m, k, ump2, proj='part', N_alpha=None, N_beta=None, N_gamma=None, 
        ...
        if proj == 'part':
            for b in range(N_beta):
                rot_mo_coeff = rot_mo_coeffs[b]
                coeff = coeffs[b]
                _energy_00 = self.get_matrix_element_v2(mo_coeff, rot_mo_coeff, mo_occ, mo_occ, verbose=verbose)
    ...
    def det_ovlp_v3(self, mo_coeff1, mo_coeff2, _mo_occ1=None, _mo_occ2=None):
        ...
        Args
            _mo_occ1, _mo_occ2 (list): List of orbital indices to indicate occupied-virtual excitations, 
                                       e.g. [i, j, a, b]
        """
        if not isinstance(self.ao_ovlp, numpy.ndarray):
            self.get_ao_ovlp()
    ...
    def get_mo_eri_complex_v2(self, mo_coeff):
        """
        Transforms the AO eris to MO eris. For complex-valued mo_coeffs.
        ...
        Returns
            mo_eri with shape (nocc, nvir, nocc, nvir)
        """
        if not isinstance(self.mo_eri, numpy.ndarray):
            ao_eri = self.get_ao_eri()
            nocc = self.nelec
            ...
            self.mo_eri = gmp2_slow.ao2mo_slow(ao_eri, (orboa,orbva,orboa,orbva))
            self.mo_eri += gmp2_slow.ao2mo_slow(ao_eri, (orbob,orbvb,orbob,orbvb)) 
            self.mo_eri1 = gmp2_slow.ao2mo_slow(ao_eri, (orboa,orbva,orbob,orbvb))
            self.mo_eri += self.mo_eri1
            self.mo_eri += self.mo_eri1.transpose(2,3,0,1) 
    ...
    def get_mo_eri_with_df(self, dfgmp2, mo_coeff):
        """
        Transforms the AO eris to MO eris using density fitting. For real- or complex-valued mo_coeffs.
        ...
        nocc = self.nelec
        nmo = self.nmo
        naux = dfgmp2.with_df.get_naoaux()
        
        mo_eri = numpy.empty((nmo, nmo, nmo, nmo), dtype=mo_coeff.dtype)
        La = numpy.empty((naux, nmo**2), dtype=mo_coeff.dtype)
        Lb = numpy.empty((naux, nmo**2), dtype=mo_coeff.dtype)
        
        p1 = 0
        for qa, qb in self.loop_ao2mo(dfgmp2, mo_coeff, nocc):
            p0, p1 = p1, p1 + qa.shape[0]
            La[p0:p1] = qa
            Lb[p0:p1] = qb
            
        for i in range(nmo):
            mo_eri_i = numpy.dot(La[:,i*nmo:(i+1)*nmo].T, La)
            mo_eri_i += numpy.dot(Lb[:,i*nmo:(i+1)*nmo].T, Lb)
            mo_eri_i += numpy.dot(La[:,i*nmo:(i+1)*nmo].T, Lb)
            mo_eri_i += numpy.dot(Lb[:,i*nmo:(i+1)*nmo].T, La)
            mo_eri_i = mo_eri_i.reshape(nmo, nmo, nmo)
            mo_eri[i] = mo_eri_i
            
        self.mo_eri_df = mo_eri
        return mo_eri
            
    
    def get_mo_eri_with_df_v2(self, dfgmp2, mo_coeff):
        """
        Transforms the AO eris to MO eris using density fitting. For real- or complex-valued mo_coeffs.
        ...
        mo_eri = numpy.empty((nocc, nvir, nocc, nvir), dtype=mo_coeff.dtype)
        Lova = numpy.empty((naux, nocc*nvir), dtype=mo_coeff.dtype)
        Lovb = numpy.empty((naux, nocc*nvir), dtype=mo_coeff.dtype)
        
        p1 = 0
        for qova, qovb in dfgmp2.loop_ao2mo(mo_coeff, nocc, orbspin=None):
            p0, p1 = p1, p1 + qova.shape[0]
            Lova[p0:p1] = qova
            Lovb[p0:p1] = qovb
            
        for i in range(nocc):
            mo_eri_i = numpy.dot(Lova[:,i*nvir:(i+1)*nvir].T, Lova)
            mo_eri_i += numpy.dot(Lovb[:,i*nvir:(i+1)*nvir].T, Lovb)
            mo_eri_i += numpy.dot(Lova[:,i*nvir:(i+1)*nvir].T, Lovb)
            mo_eri_i += numpy.dot(Lovb[:,i*nvir:(i+1)*nvir].T, Lova)
            mo_eri_i = mo_eri_i.reshape(nvir, nocc, nvir)
            mo_eri[i] = mo_eri_i
    ...
    def loop_ao2mo(self, dfgmp2, mo_coeff, nocc):
        """
        WARNING: complex-orbital treatment might be incorrect.
        """
        ...
        if complex_orb:
            moa_RR = moa.real
            moa_II = moa.imag
            moa_RI = numpy.concatenate((moa.real[:,:nocc], moa.imag[:,nocc:]), axis=1)
            moa_IR = numpy.concatenate((moa.imag[:,:nocc], moa.real[:,nocc:]), axis=1)
            
            mob_RR = mob.real
            mob_II = mob.imag
            mob_RI = numpy.concatenate((mob.real[:,:nocc], mob.imag[:,nocc:]), axis=1)
            mob_IR = numpy.concatenate((mob.imag[:,:nocc], mob.real[:,nocc:]), axis=1)
        ...
        with_df = dfgmp2.with_df

        nvir = nmo - nocc
        naux = with_df.get_naoaux()
        mem_now = lib.current_memory()[0]
        max_memory = max(2000, dfgmp2.max_memory*.9-mem_now)
        fac = 1
        ...
        for eri1 in with_df.loop(blksize=blksize):
            if complex_orb:
                Lova_RR = ao2mo._ao2mo.nr_e2(eri1, moa_RR, ijslice, aosym='s2', out=Lova_RR)
                Lova_II = ao2mo._ao2mo.nr_e2(eri1, moa_II, ijslice, aosym='s2', out=Lova_II)
                Lova_RI = ao2mo._ao2mo.nr_e2(eri1, moa_RI, ijslice, aosym='s2', out=Lova_RI)
                Lova_IR = ao2mo._ao2mo.nr_e2(eri1, moa_IR, ijslice, aosym='s2', out=Lova_IR)
                Lova = Lova_RR + Lova_II + 1j * (Lova_RI - Lova_IR)
                
                Lovb_RR = ao2mo._ao2mo.nr_e2(eri1, mob_RR, ijslice, aosym='s2', out=Lovb_RR)
                Lovb_II = ao2mo._ao2mo.nr_e2(eri1, mob_II, ijslice, aosym='s2', out=Lovb_II)
                Lovb_RI = ao2mo._ao2mo.nr_e2(eri1, mob_RI, ijslice, aosym='s2', out=Lovb_RI)
                Lovb_IR = ao2mo._ao2mo.nr_e2(eri1, mob_IR, ijslice, aosym='s2', out=Lovb_IR)
                Lovb = Lovb_RR + Lovb_II + 1j * (Lovb_RI - Lovb_IR)
                    
            else:
                Lova = ao2mo._ao2mo.nr_e2(eri1, moa, ijslice, aosym='s2', out=Lova)
                Lovb = ao2mo._ao2mo.nr_e2(eri1, mob, ijslice, aosym='s2', out=Lovb)
    ...
    def get_co_eri(self, co_coeff):
        """
        Transforms AO eris to CO eris. For complex-valued co_coeffs.

        Returns
            co_eri with shape (nocc, nvir, nocc, nvir)
        """
        ao_eri = self.get_ao_eri()
        nocc = self.nelec
        nao = self.mol.nao
        ...
        co_eri = gmp2_slow.ao2mo_slow(ao_eri, (orboa,orbva,orboa,orbva))
        co_eri += gmp2_slow.ao2mo_slow(ao_eri, (orbob,orbvb,orbob,orbvb)) 
        co_eri1 = gmp2_slow.ao2mo_slow(ao_eri, (orboa,orbva,orbob,orbvb))
        co_eri += co_eri1
    ...
    def get_co_eri_from_mo_eri_with_df(self, dfgmp2, mo_coeff, u_oo):
        """
        Computes the CO eris from the MO eris. The orbitals are from |Psi_{UHF}>.
        ...
        """
        if not isinstance(self.mo_eri, numpy.ndarray):
                self.get_mo_eri_with_df(dfgmp2, mo_coeff)
            
        # (nocc, nvir, nocc, nvir)
        mo_eri_ovov = self.mo_eri_df[:self.nelec, self.nelec:, :self.nelec, self.nelec:]
        return numpy.einsum('pi,qj,paqb->iajb', u_oo.conj(), u_oo.conj(), mo_eri_ovov)
...
def build_chain(n, bond_length, basis='sto-6g'):
    """
